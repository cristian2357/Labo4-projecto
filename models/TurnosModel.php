...
class TurnosModel extends Model
{
    public function insertarTurno($idEmpresa, $idSucursal, $hora, $fecha, $cliente)
    {
        ...
        $this->db->query("INSERT into turnos (idsucursales, idempresas, horario, fecha, idcliente)
            values ($idSucursal, $idEmpresa, str_to_date('$hora','%H:%i'),
            str_to_date('$fecha','%d/%m/%y'), $idCliente)");
